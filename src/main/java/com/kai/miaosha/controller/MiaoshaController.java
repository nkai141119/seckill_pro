...
import com.kai.miaosha.domain.MiaoshaOrder;
import com.kai.miaosha.domain.MiaoshaUser;
import com.kai.miaosha.domain.OrderInfo;
import com.kai.miaosha.rabbitmq.MQSender;
import com.kai.miaosha.rabbitmq.MiaoshaMessage;
import com.kai.miaosha.redis.AccessKey;
import com.kai.miaosha.redis.GoodsKey;
import com.kai.miaosha.redis.RedisService;
import com.kai.miaosha.result.CodeMsg;
import com.kai.miaosha.result.Result;
import com.kai.miaosha.service.GoodsService;
import com.kai.miaosha.service.MiaoshaService;
import com.kai.miaosha.service.MiaoshaUserService;
import com.kai.miaosha.service.OrderService;
...
public class MiaoshaController implements InitializingBean{
	 ...
	 */
	//POST请求
	//本方法负责预减库存、发消息到mq队列，返回code(0)表示已经开始排队。不再执行真正的mysql减库存、生成订单秒杀。
	//mysql判定库存、查重复单在MQReceiver，减库存、创建订单在MiaoshaService.miaosha1
...
//		}
		...
		//5.可以秒杀，正常请求，入队，发送一个秒杀message到队列里面去，入队之后客户端应该进行轮询。
	...
	public String toList(Model model,MiaoshaUser user,@RequestParam("goodsId") Long goodsId) {
		...
		return "order_detail";//返回订单详情页面，不推荐，影响性能
	}
	
	
	/**
	 * 
	 * 做了页面静态化的，直接返回订单的信息而不是页面
	 * 但是还没有做ajaxcache，仍旧不是最新版本
	 * 也没有使用Redis预减库存
	 * @param model
	 * @param user
	 * @param goodsId
