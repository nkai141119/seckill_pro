...
import com.kai.miaosha.dao.MiaoshaUserDao;
import com.kai.miaosha.domain.MiaoshaUser;
import com.kai.miaosha.exception.GlobalException;
import com.kai.miaosha.redis.MiaoshaUserKey;
import com.kai.miaosha.redis.RedisService;
import com.kai.miaosha.result.CodeMsg;
import com.kai.miaosha.util.MD5Util;
import com.kai.miaosha.util.UUIDUtil;
...
public class MiaoshaUserService {
	...
	public MiaoshaUser getById(long id) {
		//1.取缓存	---先根据id来取得缓存 拼装出的realkey 为 MiaoshaUserKey:idid (后一个id是具体的id），并且返回
		//类型会转化为指定的MiaoshaUser.class
		MiaoshaUser user=redisService.get(MiaoshaUserKey.getById, ""+id, MiaoshaUser.class);
		//能再缓存中拿到
		if(user!=null) {
			return user;
		}
		//2.缓存中拿不到，那么就去取数据库，这个函数参数相对简单
		user=miaoshaUserDao.getById(id);
		//3.如果数据库查到了，返回的同时把MiaoshaUser缓存到Redis中，方便下一次查询
		if(user!=null) {
	...
	//在LoginController中的dologin_test调用loginString
...
//		return CodeMsg.SUCCESS;
	...
	public CodeMsg login(HttpServletResponse response,LoginVo loginVo) {
		...
		String mobile=loginVo.getMobile();
		String formPass=loginVo.getPassword();
		//判断手机号是否存在，默认会先去Redis查询，没有再去mysql
		MiaoshaUser user=getById(Long.parseLong(mobile));
		//查询不到该手机号的用户
		if(user==null) {
	...
	public void addCookie(MiaoshaUser user,String token,HttpServletResponse response) {
		...
		// 设置网站的根目录，服务器上所有的应用都能够访问到这个cookie
		cookie.setPath("/");
		// 需要写到response中
