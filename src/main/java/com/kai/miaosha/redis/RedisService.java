...
public class RedisService {
	...
	//所以需要单独拿出JedisPool的bean作为RedisPoolFactory
	/**
	 * 获取单个对象
	 * @param prefix
	 * @param key
	...
	public <T> T get(KeyPrefix prefix,String key,Class<T> data){
		...
		try {
			...
			//生成真正的key  className+":"+prefix;  例如UserKey:namezhangkai
	...
	public <T> boolean set(KeyPrefix prefix,String key,T value){
		...
		try {//在JedisPool里面取得Jedis
			...
			String s=beanToString(value);//将T类型转换为String类型，基本类型还是基本类型string，复杂类型则调用FastJson转化为JSONString
			//System.out.println("s:"+s);
			if(s==null||s.length()<=0) {
				return false;
			}
			int seconds=prefix.expireSeconds();
			if(seconds<=0) {//有效期小于等于0：代表不过期
				jedis.set(realKey, s);
			}else {//有过期时间，设置过期时间
	...
	public static <T> String beanToString(T value) {
		...
		if(clazz==int.class||clazz==Integer.class) {
			...
			//如果不是以上类型，就直接调用fastjson的处理方式
			return JSON.toJSONString(value);
		}		
	}
	//关闭jedis的方法——实质上不会关掉而是返回到连接池
